    ...
    function renderTopUsers(users) {
        ...
        users.forEach((user, index) => {
            ...
            topUserElement.innerHTML = `
                ...
                <img src="assets/images/user${user.id}.jpg" alt="${user.username}" class="profile-pic">
                <div class="user-info">
                    <h3>${user.username}</h3>
    ...
    function renderTableRows(users) {
        ...
        users.forEach((user, index) => {
            ...
            row.innerHTML = `
                <td>${index + 6}</td>
                <td>
                    <img src="assets/images/user${user.id}.jpg" alt="${user.username}" class="profile-pic-small">
                    ${user.username}
                </td>
                <td>${user.country}</td>
                <td>${user.points}</td>
